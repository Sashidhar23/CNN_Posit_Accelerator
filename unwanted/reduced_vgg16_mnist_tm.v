...
module reduced_vgg16_mnist_tm #(
    ...
    localparam [4:0] ST_SETUP_LAYER       = 5'd1;
    localparam [4:0] ST_INIT_TILE         = 5'd2;
    localparam [4:0] ST_LOAD_TILE         = 5'd3;
    localparam [4:0] ST_STREAM_TILE       = 5'd4;
    localparam [4:0] ST_NEXT_DOT          = 5'd5;
    localparam [4:0] ST_NEXT_OC           = 5'd6;
    localparam [4:0] ST_POST_DIRECT_SET   = 5'd7;
    localparam [4:0] ST_POST_DIRECT_WAIT  = 5'd8;
    localparam [4:0] ST_POST_DIRECT_WRITE = 5'd9;
    localparam [4:0] ST_POST_POOL_SET     = 5'd10;
    localparam [4:0] ST_POST_POOL_WAIT    = 5'd11;
    localparam [4:0] ST_POST_POOL_CAPTURE = 5'd12;
    localparam [4:0] ST_ARGMAX_SET        = 5'd13;
    localparam [4:0] ST_ARGMAX_WAIT       = 5'd14;
    localparam [4:0] ST_ARGMAX_CAPTURE    = 5'd15;
    localparam [4:0] ST_DONE              = 5'd16;
    localparam [4:0] ST_STORE_LOGITS      = 5'd17;
    localparam [4:0] ST_PREP_ACT_READ     = 5'd18;
    localparam [4:0] ST_PREP_ACT_CAPTURE  = 5'd19;
    localparam [4:0] ST_PREP_ACT_WAIT     = 5'd20;

    localparam [N-1:0] POSIT_ZERO = {N{1'b0}};
    localparam [N-1:0] POSIT_NAR  = {1'b1, {(N-1){1'b0}}};

    (* ram_style = "block" *) reg [N-1:0] feature_a [0:MAX_FEATURE_VALUES-1];
    ...
    (* ram_style = "ultra" *) reg [N-1:0] weight_mem [0:TOTAL_WEIGHT_VALUES-1];
    (* ram_style = "block" *) reg [N-1:0] bias_mem [0:TOTAL_BIAS_VALUES-1];
    ...
    reg feature_a_we;
    reg [31:0] feature_a_write_addr;
    reg [N-1:0] feature_a_write_data;
    reg [31:0] feature_a_read_addr;
    reg [N-1:0] feature_a_read_data;
    ...
    always @(posedge clk) begin
        if (feature_a_we && feature_a_write_addr < MAX_FEATURE_VALUES)
            feature_a[feature_a_write_addr] <= feature_a_write_data;

        if (feature_a_read_addr < MAX_FEATURE_VALUES)
            feature_a_read_data <= feature_a[feature_a_read_addr];
        else
            feature_a_read_data <= POSIT_ZERO;
    end

    always @(posedge clk) begin
        if (!busy) begin
            if (cfg_write_en && cfg_mem == CFG_WEIGHT &&
                cfg_layer < 4'd12 && cfg_addr < layer_weight_size(cfg_layer))
                weight_mem[layer_weight_base(cfg_layer) + cfg_addr] <= cfg_data;

            if (cfg_write_en && cfg_mem == CFG_BIAS &&
                cfg_layer < 4'd12 && cfg_addr < layer_bias_size(cfg_layer))
                bias_mem[layer_bias_base(cfg_layer) + cfg_addr] <= cfg_data;
        end
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            feature_a_we <= 1'b0;
            feature_a_write_addr <= 0;
            feature_a_write_data <= POSIT_ZERO;
            feature_a_read_addr <= MAX_FEATURE_VALUES;
        end
        else begin
            done <= 1'b0;
            feature_a_we <= 1'b0;
            ...
            if (!busy && cfg_write_en && cfg_mem == CFG_INPUT &&
                ...
                feature_a_we <= 1'b1;
                feature_a_write_addr <= cfg_addr;
                feature_a_write_data <= cfg_data;
            end
            ...
            case (state)
                ...
                ST_INIT_TILE: begin
                    if (init_col < COLS && (oc_base + init_col) < cur_out_ch) begin
                        ...
                        if (out_index < COLS*MAX_OUT_PIXELS)
                            tile_out[out_index] <= bias_mem[cur_bias_base + oc_base + init_col];
                    end
                ...
                ST_LOAD_TILE: begin
                    if (load_index < ROWS*COLS) begin
                        ...
                        if ((dot_idx < cur_dot_len) && (out_ch_idx < cur_out_ch))
                            core_weight_data <= weight_mem[cur_weight_base + (out_ch_idx * cur_dot_len) + dot_idx];
                        else
                            core_weight_data <= POSIT_ZERO;
                ...
                ST_PREP_ACT_READ: begin
                    if (stream_send_count < cur_out_pixels) begin
                        feature_a_read_addr <= activation_addr(dot_base + prep_row,
                                                               stream_y,
                                                               stream_x);
                ...
                ST_PREP_ACT_WAIT: begin
                    state <= ST_PREP_ACT_CAPTURE;
                end

                ST_PREP_ACT_CAPTURE: begin
                    core_activation_vector_data[prep_row*N +: N] <= feature_a_read_data;
                ...
                ST_POST_DIRECT_WRITE: begin
                    ...
                    if (dest_index < MAX_FEATURE_VALUES) begin
                        feature_a_we <= 1'b1;
                        feature_a_write_addr <= dest_index;
                        feature_a_write_data <= relu_single_out;
                    end
                    post_idx <= post_idx + 1;
                    state <= ST_POST_DIRECT_SET;
                end
                ...
                ST_POST_POOL_CAPTURE: begin
                    if (pool_pos == 2'd3) begin
                        ...
                        if (dest_index < MAX_FEATURE_VALUES) begin
                            feature_a_we <= 1'b1;
                            feature_a_write_addr <= dest_index;
                            feature_a_write_data <= pool_single_out;
                        end
