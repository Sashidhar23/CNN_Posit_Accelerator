...
module tb_conv2D_stream;
    ...
    initial begin
        ...
        if (!done) begin
            $display("FAIL timeout waiting for streamed conv2D done");
            errors = errors + 1;
        end
        ...
        if (errors == 0)
            $display("tb_conv2D_stream PASS");
        else
            $display("tb_conv2D_stream FAIL errors=%0d", errors);
