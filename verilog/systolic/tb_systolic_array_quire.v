...
module tb_systolic_array_quire;
    ...
    initial begin
        ...
        if (psum_out !== {COLS{POSIT_ZERO}} || psum_nar_out !== {COLS{1'b0}}) begin
            $display("FAIL reset clears quire array outputs psum=%h nar=%b", psum_out, psum_nar_out);
            errors = errors + 1;
        end
        else begin
            $display("PASS reset clears quire array outputs");
        end
        ...
        for (cycle = 0; cycle < 40 && seen_cols !== {COLS{1'b1}}; cycle = cycle + 1) begin
            ...
            for (i = 0; i < COLS; i = i + 1) begin
                if (psum_out[i*N +: N] === POSIT_ONE && psum_nar_out[i] === 1'b0)
                    seen_cols[i] = 1'b1;
            end
        end
        pe_en = 1'b0;
        ...
        if (seen_cols !== {COLS{1'b1}}) begin
            $display("FAIL row0 activation did not reach every quire column seen=%b final=%h nar=%b",
                     seen_cols, psum_out, psum_nar_out);
            errors = errors + 1;
        end
        else begin
            $display("PASS row0 activation reaches each raw quire bottom rounded psum");
        end
