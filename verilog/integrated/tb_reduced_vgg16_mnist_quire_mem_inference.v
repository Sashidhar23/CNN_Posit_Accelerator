...
module tb_reduced_vgg16_mnist_quire_mem_inference;
    localparam N = 8;
    localparam ES = 1;
    localparam IN_CH = 1;
    localparam IN_H = 28;
    ...
    // Convert the supplied parameter export to the standard Posit byte
    // convention used by posit_decoder.  Pixel bytes are already standard.
...
`include "tb/cnn/mnist_expected_labels.vh"
        ...
        if (errors == 0)
            $display("tb_reduced_vgg16_mnist_quire_mem_inference PASS");
        else
            $display("tb_reduced_vgg16_mnist_quire_mem_inference FAIL errors=%0d", errors);
